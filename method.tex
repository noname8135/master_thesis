...
To analyze the time complexity, let the number of all entries in the network be $E$, average number of entries in each switch be $ES$, average number of entries in each aggregated group be $EG$, and number of aggregated groups be $G$. In each level of finding an aggregated group, we need $O(ES)$ for checking if the detection packet matches any entry on the switch. After ensuring that no such entry exists on a switch, for an entry on the switch, $EG$ is needed to check if the entry fits without any conflicted fields, and another $EG$ to check if there exists any entry with the same field and column on switches on which the entries already in this group are, which means we need $O(ES*(2*EG))$ for checking through all the entries on a switch. Since visited switch and entry checking are implemented by set in Python, they will take constant time. In total, the time spent in each depth will be $O(ES) + O(ES*EG)$, which equals to $O(ES*EG)$. Thus, the total time complexity of all aggregated groups finding will be $O(ES*EG) * EG * G$, which equals to $O(ES*EG^2*G)$. And since $EG$ is a number less than 6, we can further reduce the total time to $ES*G$. Since $G$ \texttt{Packet\_Out}s will be sent, the time for the \texttt{Packet\_In} checking process is between $O(G)$.
