...
\item
There is no switch that has more than flow one entry selected in the same group. \red{Given that we have said ``one selected flow entry on each switch in the same group'' in the last paragraph, is the condition still needed?}.
\item
The forged packets should be valid. 
\end{enumerate}
