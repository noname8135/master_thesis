...
Assuming an attacker is able to add, remove, or modify the entries in the flow tables of the compromised
...
In our method, there is another important consideration: the controller must self-maintain unpolluted information of all flow entries. It is not reliable for a controller to obtain flow entry information by querying switches, because a switch might be malicious and forge a fake response that gives false information about flow entry.
\end{enumerate}
...
In order to optimize the number of necessary detection packets, the whole network should be covered with the minimum number of aggregated groups. The graph problem is more complex than Longest path problem, which is 
