...
This work presents a method of detecting a compromised switch, and focuses on the problem that a compromised switch will bring. The possibilities of how attacks \red{general SDN attacks or just compromising a switch?} happen have been already discussed in Section~\ref{SDN security}. As a defender, we will focus on detection methods of certain scenarios \red{Don't use `certain'. Be specific.}. There are two parts in our work \red{What two parts? Name them here.}. We will define the threat model both parts share, and both parts will be explained along with their attack scenarios and detection algorithm.
...
\item
OpenFlow version 1.3 is used as the southbound protocol \red{Is version 1.3 necessary in the assumption?}.
\item
The switches is able to access the Internet. 
\item
Only one OpenFlow switch is compromised. No cooperation for attack will happen between multiple entities inside SDN network. \red{You repeat the first item here.}
\item
...
\item
An attacker cannot totally change the way of switch processing or core mechanism, but only perform attack by manipulating some elements \red{What elements? Be specific.} .
\item
...
switch without notifying the controller, so packets may be forwarded to an undesired destination. The proposed method is intended to detect this behavior with high efficiency. In this method, only output action of packets is considered, other actions such as dropping packets, setting field, changing TTL etc. will be ignored \red{Cann't they (e.g., dropping packets) be detected?}. 

It is certainly possible that more than one switch are made by the same manufacturer or have the same software version in the same network, so multiple switches share the same vulnerabilities and may be compromised simultaneously. However, cooperation between multiple compromised switches complicates the scenario significantly. Taking our own method as an example, suppose there are two compromised switches, CS1 and CS2, which are neighbors to each other, and both contain an entry in the same aggregation group \red{You haven't defined aggregation group here}. When CS1 receives a detection packet and forwarded to other place maliciously, it sends a copy to CS2, and the rest of the detection process will continue without raising any suspicion \red{Since the controller does not receive the detection packet, is it possible to raise a suspicion here?}. In order to simplify the case as a starting point for developing the detection method, we assume only one switch inside the network is compromised.
