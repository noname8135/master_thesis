...
%  \begin{algorithmic}[1]
%    \Require
%      $switches$: the set of switches, each of which contains a set of entries;  \newline
%      %$entry$: match field, value and destination of an entry;  \newline
%      $packet$: the detection packet, where $packet[field]$ denotes the $field$ in $packet$; \newline
%      $aggregated\_group$: contains a set of entries in an aggregated group; \newline
%      
%    \Function{find\_aggregated\_groups}{$switches$}
%      \State $group\_id \gets 0$;
%      \While{not all the entries have been visited}
%            \State $starting\_switch \gets \textrm{an arbitrary switch with unvisited entry from}\; %switches$;
%            \State $packet \gets empty$;   // cleared per round
%            \State $aggregated\_group \gets empty$;
%            \State $packet[vid] \gets group\_id$;   // VID as the group identifier 
%            \State $group\_id \gets group\_id + 1$;
%            \State $packet \gets \Call{find\_one\_group}{starting\_switch,packet}$;
%            \State Send$\;packet\;$with \texttt{Packet\_Out};
%      \EndWhile
%    \EndFunction
%  \end{algorithmic}
%\end{algorithm}
