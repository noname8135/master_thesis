...
\label{SDN security}
A thorough review of SDN-related security issues can be found in \red{Cite a few SDN security survey papers; not hard to find in IEEE Xplore}. Therefore, rather than repeat the review, we will focus on the issue of compromising OpenFlow switches, since it has been less addressed than others so far. Compromising OpenFlow switches can lead to some bad results. (1) \red{Finish this sentence.} topology poisoning. (2) The actions specified in the flow rules on a compromised switch can be unexpected altered. (3) The packets through a compromised switch can be eavesdropped. \red{enumerate more (4) ...} \red{Please rewrite the content until the end of this section to discuss existing research related to addressing these results.}
